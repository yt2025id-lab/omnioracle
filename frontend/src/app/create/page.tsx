...
const CATEGORIES = ["Crypto", "Sports", "Politics", "Science", "Entertainment", "Custom"] as const;
...
type NoticeType = "idle" | "error" | "info" | "success";

export default function CreateMarketPage() {
  const { isConnected } = useAccount();
  const { writeContractAsync, data: txHash, isPending: isWalletPending } = useWriteContract();
  ...
  if (isConfirmed && noticeType !== "success") {
    setSuccess("Transaction confirmed. Market creation succeeded.");
  }

  if (receiptError && noticeType !== "error") {
          ...
          {PIPELINE_OPTIONS.map((p) => (
            <button
              ...
              style={{
                ...
                transform:
                  selectedPipeline === p.type ? "translateX(6px)" : "translateX(0)",
              }}
            >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                    <span
                      style={{
                        ...
                        fontSize: 14,
                        color:
                          selectedPipeline === p.type ? p.color : "var(--text-primary)",
                      }}
                    >
                      {p.label}
                    </span>
